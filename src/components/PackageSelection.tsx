...
export default function PackageSelection({ 
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to update assessment tier');
      }
      ...
      alert('Failed to update package selection. Please try again.');
    } finally {
      setIsUpdating(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto py-8 px-4">
      ...
      <div className="mt-8 flex justify-center">
        ...
        >
          {isUpdating ? 'Updating...' : 'Continue to Payment'}
        </button>
      </div>
    </div>
  );
