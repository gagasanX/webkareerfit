...
export default function AssessmentPage() {
  ...
  const [selectedTier, setSelectedTier] = useState<string>('standard'); // Default to standard tier
              ...
              <div>
                  ...
                  {pricingTiers.map((tier) => (
                    <div 
                      ...
                      onClick={() => {
                        ...
                        setCouponApplied(false); // Reset coupon when tier changes
                        setCouponError('');
                ...
                <div className="flex mt-4">
                  ...
                  <button
                    ...
                    className={`px-4 py-2 ${couponApplied ? 'bg-gray-400' : 'bg-[#38b6ff] hover:bg-[#7e43f1]'} text-white rounded-r-lg transition-colors`}
                  >
                ...
                {couponApplied && (
                  <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded-md">
                    <p className="text-green-600 text-sm">
                      Coupon applied! {discountedPrice === 0 ? 'Your assessment is now free.' : `New price: RM ${discountedPrice}`}
                    </p>
                  </div>
                )}
                ...
                <button
                  ...
                  className={`mt-4 w-full py-3 ${
                    loading ? 'bg-gray-400' : 
                    discountedPrice === 0 && couponApplied
                      ? 'bg-green-500 hover:bg-green-600' 
                      : 'bg-gradient-to-r from-[#38b6ff] to-[#7e43f1] hover:from-[#7e43f1] hover:to-[#38b6ff]'
                  } text-white font-medium rounded-lg transition-all duration-300 shadow-lg hover:shadow-xl`}
                >
                  {loading ? 'Processing...' : 
                   discountedPrice === 0 && couponApplied
                    ? 'Start Free Assessment' 
                    : `Start Assessment for RM ${couponApplied ? discountedPrice : basePrice}`}
                </button>
