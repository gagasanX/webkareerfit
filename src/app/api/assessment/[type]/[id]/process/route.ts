...
export async function POST(
  ...
  try {
    ...
    console.log(`Starting AI processing for assessment: ${assessment.id} (${(await params).type})`);
    ...
    try {
      ...
      const analysisResult = await callOpenAI((await params).type, responses);
      ...
      sendAssessmentEmail({
        ...
        assessmentType: (await params).type
      }).catch((emailError) => {
      ...
      const fallbackAnalysis = createFallbackAnalysis((await params).type, responses);
    ...
    try {
      ...
      if (assessment) {
        await updateAssessment((await params).id, 'error', {
...
async function validateRequest(params: { type: string, id: string }) {
  if (!params || !(await params).id || !(await params).type) {
    return { 
