...
export async function GET(
  ...
  try {
    ...
    if (!params || typeof (await params).id !== 'string' || typeof (await params).type !== 'string') {
      console.error('Invalid parameters:', params);
      return NextResponse.json({ error: 'Invalid parameters' }, { status: 400 });
    }
    
    const assessmentId = (await params).id.trim();
    ...
    if (assessment.manualProcessing || 
      ...
      return NextResponse.json({
        ...
        redirectUrl: assessment.tier === 'premium' || assessment.price >= 250
          ? `/assessment/${(await params).type}/premium-results/${assessmentId}`
          : `/assessment/${(await params).type}/standard-results/${assessmentId}`
      });
    }
    ...
    if (status === 'completed' || analysisStatus === 'completed') {
      ...
      if (data.processingStartedAt || data.aiProcessingStarted) {
        ...
        if (elapsedMinutes > 5) {
          ...
          fetch(`${process.env.NEXT_PUBLIC_APP_URL || 'https://my.kareerfit.com'}/api/assessment/${(await params).type}/${assessmentId}/combine-results`, {
            method: 'POST',
