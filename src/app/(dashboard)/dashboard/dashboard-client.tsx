...
export default async function DashboardPage() {
  try {
    ...
    // Based on the ClientHeader component you shared, we need to prepare the specific props it needs
    const pendingAssessments = userData.assessments.filter(
      assessment => assessment.status === 'pending'
    );
    
    const pendingPayments = userData.assessments.filter(
      assessment => assessment.payment?.status === 'pending'
    );

    // Data is loaded server-side, now we can render the client component without passing userData
    // DashboardClient likely has its own data fetching mechanism
